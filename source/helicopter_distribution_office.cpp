...
// v1.0.0 release, target SOVIET64.exe v1.1.1.7, TesmioLoader API 3
//
// Four deliberately narrow additions:
//  1) permit helicopters in a Distribution Office/AIRPLANE building by routing
//     helicopter vehicle compatibility through the same native helipad helper
//     used by the vanilla Helicopter Construction Office;
//  2) keep the two native HELIPORT_STATION records permanently in the Workshop
//     asset so placed buildings receive normal game-owned helicopter pads. Only
//     while the native bottom-menu classifier runs, temporarily expose an empty
//     station vector for this one descriptor, then restore its real vector end;
//  3) graft the same native Soviet/Western overseas-target buttons used by the
//     Dock Distribution Office onto this building's normal Distribution Office
//     panel. Clicking either button creates a normal native DO assignment record
//     for the game's permanent overseas pseudo-building;
//  4) reject cargo-heliport DO tasks for non-helicopter vehicles owned by this
//     mixed office. The task itself remains native and available to helicopters.
//
// The plugin never moves construction-menu icons or rewrites menu vectors. The
// asset always owns its two helipad records; the classifier alone sees them hidden
// for the duration of its call. If the mask cannot be applied, the pads stay intact
// and native menu nesting is allowed to fail without sacrificing functional helipads.
// Ordinary road jobs and the normal Distribution Office planner remain native.
                                              ...
                                              void** trampoline, const char* label)
{
    ...
    while (stolen < 14)
    {
        ...
        {
            if (H->log) H->log("helido    task filter hook refused: unsupported prologue at +0x%llX",
                               (u64)stolen);
            return 0;
        }
        stolen += len;
...
EXPORT int TsmPluginInit(const TsmHost* host, TsmPluginInfo* info)
{
    ...
    {
        if (H->log) H->log("helido    FAILED mixed-fleet task filter hook - refusing partial HDO load");
        return 1;
    }
    ...
    {
        if (H->log) H->log("helido    FAILED menu-safe helipad mask hook - refusing partial HDO load");
        return 1;
    }

    if (H->log)
        H->log("helido    v1.0.0 initialized: helicopter distribution office support active");
    return 0;
}
